...
//Some new include files to be compatible with visual studio
#include "mufflon.h"
#include "strdup.h"
#include "basename.h"
#include "dirname.h"
...
void begin_measure(struct timeb *start);
void end_measure(MufflonContext*, struct timeb *start, struct timeb *end);
...
/*Modified MAIN*/
int mufflon_RUN(/*int argc, char **argv*/ struct Options *options, char *Src, char *Dst) {
	int a;
	int c, v;
...
/*Modified, old case to be done by the TUI part*/
/*
//	if (argc == 1) {
...
//	}
*/
	/*Edited: translate what we need*/
	m->option_flibug = options->FlyBug;
	m->option_multipass = options->FlyBugMultipass;
	m->option_prepare = options->PreparePicture;
	m->read_name = strdupNEW(Src);
	m->write_name = strdupNEW(Dst);
/*
	for (a = 1; a<argc; a++) {
		if (strcmp(argv[a], "-o") == 0) {
			a++;
			if (argc>a) {
				m->write_name = strdupNEW(argv[a]);
			}
			else {
...
*/
	...
	if (m->error_map)
		free(m->error_map);
	if (m->error_map_name) //ok
		free(m->error_map_name);
//	if (m->fdw) // NEVER FREE A FILE OR SHIT HAPPENS
//		free(m->fdw);
	if (m->flick_map) //ok
		free(m->flick_map);
	if (m->flick_map_name) //ok
		free(m->flick_map_name);
	if (m->hires_bitmap[0]) //ok
		free(m->hires_bitmap[0]);
	if (m->hires_bitmap[1]) //ok
		free(m->hires_bitmap[1]);
	if (m->inks[0])
		free(m->inks[0]);
	if (m->inks[1])
		free(m->inks[1]); //ok
	...
	if (m->mixes_g)
		free(m->mixes_g);
/*
	if (m->option_dest_pal_name)
		free(m->option_dest_pal_name);

	if (m->option_src_pal_name)
		free(m->option_src_pal_name);
*/
	if (m->papers[0])
		free(m->papers[0]);
	if (m->papers[1])
	...
	if (m->result_map)
		free(m->result_map);
	if (m->result_map_name)
		free(m->result_map_name);
	if (m->source)
		free(m->source);
	if (m->sprites[0])
		free(m->sprites[0]);
	if (m->sprites[1])
		free(m->sprites[1]);
	if (m->sprite_bitmap[0])
		free(m->sprite_bitmap[0]);
	if (m->sprite_bitmap[1])
		free(m->sprite_bitmap[1]);
	if (m->sprite_col_tab[0])
		free(m->sprite_col_tab[0]);
	if (m->sprite_col_tab[1])
		free(m->sprite_col_tab[1]);
	if (m->write_name)
		free(m->write_name);
	free(m);
	//	exit(0);
...
void convert(MufflonContext* m) {
	//Those structs are inside of WORK to be thread safe
	struct timeb start;
	struct timeb end;
...
void prepare(MufflonContext *m) {
	...
	for (y = 0; y<C64YRES; y++) {
		//("Preparing pic: Pr0ncessing line #%d   \n\033[A",y);
		for (x = 0; x<C64XRES; x++) {
			uint8_t r, g, b;
...
void create_structures(MufflonContext *m) {
	...
	m->error_map =(uint8_t*) calloc(C64XRES*C64YRES * 3, sizeof(uint8_t));
	m->flick_map = (uint8_t*)calloc(C64XRES*C64YRES * 3, sizeof(uint8_t));
	m->result_map = (uint8_t*)calloc(C64XRES*C64YRES * 3, sizeof(uint8_t));
	m->data = (uint8_t*)calloc(C64XRES*C64YRES * 3, sizeof(uint8_t));
	m->source = (uint8_t*)calloc(C64XRES*C64YRES * 3, sizeof(uint8_t));
	...
	if (!m->write_name) { m->base_name = strdupNEW(m->read_name); m->path = strdupNEW(m->read_name); }
	else { m->base_name = strdupNEW(m->write_name); m->path = strdupNEW(m->write_name); }

	m->base_name = basename(m->base_name);
	m->path = dir_name(m->path);
	...
	m->error_map_name = (char*)malloc(strlen(m->path) + strlen(m->base_name) + 25);
	sprintf(m->error_map_name, "%s/%s_errormap.bmp", m->path, m->base_name);
	m->flick_map_name = (char*) malloc(strlen(m->path) + strlen(m->base_name) + 27);
	sprintf(m->flick_map_name, "%s/%s_flickermap.bmp", m->path, m->base_name);
	m->result_map_name = (char*)malloc(strlen(m->path) + strlen(m->base_name) + 23);
	...
	case OTYPE_NUFLI:
		if (!m->write_name) {
			m->write_name = (char*)malloc(strlen(m->path) + 18);
			if (m->otype == OTYPE_MUIFLI) {
	...
	case OTYPE_BMP:
		if (!m->write_name) {
			m->write_name = (char*)malloc(strlen(m->path) + strlen(m->base_name) + 6);
...
void begin_measure(struct timeb *start) {                                                                  //set measure start point
	ftime(start);
	return;
}

void end_measure(MufflonContext* m, struct timeb *start, struct timeb *end) {                                                                    //end of measure, calculate time needed
	double time;
	double timediff;
	ftime(end);
...
void find_free_register_switches(MufflonContext *m) {
	...
	if (m->free_switches_max<8) {
		//("not enough switches for change of spritepointers. This is a serious problem and will make your pic look b0rken.\n");
	}
}
...
uint64_t find_best_colors_new(MufflonContext *m) {
	...
	for (colum = 0; colum<5; colum++) {
		for (y = 0; y<C64YRES; y += 2) {
			//("Processing normal colums: Pr0ncessing line #%d on colum %d    \n\033[A",y,colum);
...
uint64_t find_best_colors_normal_colums(MufflonContext *m) {
	...
	/* column 39 */
	for (y = 0; y<C64YRES; y += 2) {
		//("Processing colum 39: Pr0ncessing line #%d   \n\033[A",y);
		for (a = 0; a<10; a++) { row1[a] = -1; row2[a] = -1; }
	...
	for (y = 0; y<C64YRES; y += 2) {
		...
		for (test = 0; test<24; test++) {
			...
			for (sprite = spr_start[0]; sprite >= spr_end[0]; sprite--) {
				//("Processing fli bug: Pr0ncessing line #%d   %d %d %d %d    \n\033[A",y,sprite,multi1,multi2,multi3);
			...
			if (best_row1[FLISPRITE + i] >= 0 && best_row1[FLISPRITE + i] != m->fb_sprite_col[0][i][y] && y>0) {
				if (best_free_cols>0) {
					if (m->free_switches_flibug[0][best_free_ypos[i] / 2]>0) {
						...
						////("Not enough switches available for this action, should not happen2\n");
					}
				}
				else {
					//("Not enough switches available for this action, should not happen\n");
				}
			}
		}
	}
...
void interlace_find_used_colors(MufflonContext* m) {
	...
	int mix = 0;
	for (y = 0; y<C64YRES; y++) {
		//("Searching for used colors per block: Pr0ncessing line #%d\n\033[A",y);
		for (x = 0; x<C64XRES; x++) {
			best = -1;
			for (a = 0; a<m->mix_count; a++) {
...
void interlace_render(MufflonContext* m) {
	...
	for (y = 0; y<C64YRES; y++) {
		//("Rendering bitmaps: Pr0ncessing line #%d   \n\033[A",y);
		smask1 = 0;
		smask2 = 0;
...
void find_hires_colors(MufflonContext* m) {
	...
	for (y = 0; y<C64YRES; y += 2) {
		//("Searching for best paper/ink colours and combinations: Pr0ncessing line #%d   1ink1paper: %d    1ink2paper: %d    2ink1paper: %d    2ink2paper: %d\n\033[A",y,oneinkonepaper, oneinktwopaper, twoinkonepaper, twoinktwopaper);
		for (blockx = 0; blockx<C64XRES; blockx += 8) {
			paper1 = -1;
			paper2 = -1;
			ink1 = -1;
...
void find_sprite_colors_bruteforce(MufflonContext* m) {
	...
	for (blocky = 0; blocky<C64YRES; blocky += 2) {
		//("Searching for best sprite colours: Pr0ncessing line #%d   \n\033[A",blocky);
